...
Claude Opus 4.6 (CLI mode), plus Kilocode (CLI mode) — one tool, minimal context
overhead.
...
the tool has also been tested for gemini and codex.
...
4. install Kilocode CLI: <https://github.com/Kilo-Org/kilocode>

Configure them and make sure they can use your preferred billing method
(subscription or API).
...
# Note: `grey-so` is this package (not on npm yet - use local path or link for now)
```
...
If you are using API method in any of the sub-agents, always set their
respective API key here; e.g `GEMINI_API_KEY` when using Gemini in API mode.
Otherwise the given subagent might refuse to run.
